...
@staff_member_required
@require_GET
def customer_points_summary(request: HttpRequest, customer_id: str) -> HttpResponse:
    """
    Earn/Burn points summary & history for a customer.

    Data source: GET /v1/customers/:id/member_points
    """
    if not getattr(settings, "SHOPLINE_ACCESS_TOKEN", ""):
        return render(request, "shopline_customers/portal_error.html", {
            "error_title": "Configuration missing",
    ...
    client = _get_client()
    try:
        customer = client.get_customer(customer_id, fields=["id", "name", "email", "member_point_balance"])
        data = client.get_customer_member_points(customer_id, page=page_num, per_page=per_page)
    except ShoplineAPIError as e:
        if getattr(e, "status_code", None) == 404:
            return render(request, "shopline_customers/portal_error.html", {
                "error_title": "Customer not found",
        ...
        logger.exception("Shopline API error in customer_points_summary")
        return render(request, "shopline_customers/portal_error.html", {
            "error_title": "Shopline API error",
            "error_message": str(e),
            "status_code": getattr(e, "status_code", None),
    ...
    return render(request, "shopline_customers/customer_points_summary.html", {
        "customer": customer,
        "items": items,
        "pagination": pagination,
        "current_page": current_page,
